...
void IN_DrawNumber(int val, int x, int y, int digits)
{
    ...
    if (digits == 4)
    {
        ...
        V_DrawShadowedPatch(xpos + 6 - (int16_t)(patch->width) / 2 - 12, y, patch);
    }
    if (digits > 2)
    {
        if (realdigits > 2)
        {
            ...
            V_DrawShadowedPatch(xpos + 6 - (int16_t)(patch->width) / 2, y, patch);
        }
        xpos += 12;
    }
    val = val % 100;
    ...
    {
        patch = FontBNegative;
        V_DrawShadowedPatch(xpos + 6 - (int16_t)(patch->width) / 2 - 12 * (realdigits),
                            y, patch);
    }
}
