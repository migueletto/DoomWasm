...
wad_file_t *W_AddFile (const char *filename)
{
	...
	fileinfo->filepos = (int32_t)(0);
	fileinfo->size = (int32_t)(wad_file->length);
	...
	header.numlumps = (int32_t)(header.numlumps);
	...
	header.infotableofs = (int32_t)(header.infotableofs);
    ...
    filelumps = mycalloc(numfilelumps, sizeof(lumpinfo_t));
    if (filelumps == NULL)
    {
        W_CloseFile(wad_file);
    ...
    for (i = startlump; i < numlumps; ++i)
    {
        ...
        lump_p->position = (int32_t)(filerover->filepos);
        lump_p->size = (int32_t)(filerover->size);
        lump_p->cache = NULL;
