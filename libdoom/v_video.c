...
{ 
    ...
    y -= (int16_t)(patch->topoffset);
    x -= (int16_t)(patch->leftoffset);
...
#ifdef RANGECHECK
    if (x < 0
     || x + (int16_t)(patch->width) > SCREENWIDTH
     || y < 0
     || y + (int16_t)(patch->height) > SCREENHEIGHT)
    {
        I_Error("Bad V_DrawPatch");
    }
#endif

    V_MarkRect(x, y, (int16_t)(patch->width), (int16_t)(patch->height));

    col = 0;
    desttop = dest_screen + y * SCREENWIDTH + x;

    w = (int16_t)(patch->width);
...
void V_DrawPatchFlipped(int x, int y, patch_t *patch)
{
    ...
    for ( ; col<w ; x++, col++, desttop++)
    {
        column = (column_t *)((byte *)patch + (int32_t)(patch->columnofs[w-1-col]));
                  ...
                  byte *palette)
{
    ...
    pcx->palette_type = (int16_t)(2);	// not a grey scale
