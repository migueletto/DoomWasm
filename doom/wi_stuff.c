...
// NET GAME STUFF
#define NG_STATSY		50
#define NG_STATSX		(32 + (int16_t)(star->width)/2 + 32*!dofrags)
...
void WI_drawLF(void)
{
    ...
    {
        // draw <LevelName> 
        V_DrawPatch((SCREENWIDTH - (int16_t)(lnames[wbs->last]->width))/2,
                    y, lnames[wbs->last]);

        // draw "Finished!"
        y += (5*(int16_t)(lnames[wbs->last]->height))/4;

        V_DrawPatch((SCREENWIDTH - (int16_t)(finished->width)) / 2, y, finished);
    }
...
void WI_drawEL(void)
{
    ...
    // draw "Entering"
    V_DrawPatch((SCREENWIDTH - (int16_t)(entering->width))/2,
		y,
                entering);

    // draw level
    y += (5*(int16_t)(lnames[wbs->next]->height))/4;

    V_DrawPatch((SCREENWIDTH - (int16_t)(lnames[wbs->next]->width))/2,
		y, 
                lnames[wbs->next]);
  ...
  patch_t*	c[] )
{
    ...
    do
    {
	left = lnodes[wbs->epsd][n].x - (int16_t)(c[i]->leftoffset);
	top = lnodes[wbs->epsd][n].y - (int16_t)(c[i]->topoffset);
	right = left + (int16_t)(c[i]->width);
	bottom = top + (int16_t)(c[i]->height);
  ...
  int		digits )
{
    ...
    int		fontwidth = (int16_t)(num[0]->width);
    int		neg;
    int		temp;
  ...
  int		t )
{
	...
	{
	    n = (t / div) % 60;
	    x = WI_drawNum(x, y, n, 2) - (int16_t)(colon->width);
	    div *= 60;
    ...
    {
	// "sucks"
	V_DrawPatch(x - (int16_t)(sucks->width), y, sucks); 
    }
}
...
void WI_drawDeathmatchStats(void)
{
    ...
    V_DrawPatch(DM_TOTALSX-(int16_t)(total->width)/2,
	...
	if (playeringame[i])
	{
	    V_DrawPatch(x-(int16_t)(p[i]->width)/2,
			DM_MATRIXY - WI_SPACINGY,
			p[i]);
	    
	    V_DrawPatch(DM_MATRIXX-(int16_t)(p[i]->width)/2,
			y,
			p[i]);
	...
	{
	    // V_DrawPatch(x-(int16_t)(bp[i]->width)/2,
	    //   DM_MATRIXY - WI_SPACINGY, bp[i]);
	    // V_DrawPatch(DM_MATRIXX-(int16_t)(bp[i]->width)/2,
	    //   y, bp[i]);
	}
	x += DM_SPACINGX;
	y += WI_SPACINGY;
...
void WI_drawNetgameStats(void)
{
    ...
    int		x;
    int		y;
    int		pwidth = (int16_t)(percent->width);
    ...
    // draw stat titles (top line)
    V_DrawPatch(NG_STATSX+NG_SPACINGX-(int16_t)(kills->width),
		NG_STATSY, kills);

    V_DrawPatch(NG_STATSX+2*NG_SPACINGX-(int16_t)(items->width),
		NG_STATSY, items);

    V_DrawPatch(NG_STATSX+3*NG_SPACINGX-(int16_t)(secret->width),
		NG_STATSY, secret);
    
    if (dofrags)
	V_DrawPatch(NG_STATSX+4*NG_SPACINGX-(int16_t)(frags->width),
		    NG_STATSY, frags);

    // draw stats
    y = NG_STATSY + (int16_t)(kills->height);
	...
	x = NG_STATSX;
	V_DrawPatch(x-(int16_t)(p[i]->width), y, p[i]);

	if (i == me)
	    V_DrawPatch(x-(int16_t)(p[i]->width), y, star);
